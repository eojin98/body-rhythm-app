    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="card card-body" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
          ...
          <TimeInputField value={sleepTime} onChange={onSleepChange} defaultValue="23:00" />
        </div>
        <div style={{ height: 1, background: '#EEE' }} />
        <div>
          <div className="input-label" style={{ marginBottom: 10 }}>기상 시간 (오늘 아침)</div>
          <TimeInputField value={wakeTime} onChange={onWakeChange} defaultValue="07:00" />
        </div>
      </div>
        ...
        {MEAL_ITEMS.map(({ key, label, icon }, i) => {
          ...
          return (
            ...
            >
              ...
              {/* TimeInputField — only when not skipped */}
              {!meal.skipped && (
                <TimeInputField
                  value={meal.time}
